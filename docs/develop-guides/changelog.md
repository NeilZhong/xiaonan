...
- 侧边栏模块合并精简：智能体统一命名「数字警员」（入口收敛为 `/agent-manage`，兼容旧路由 `activePaths` 高亮；「协助伙伴」「我的数字警员」不再独立占位）；审核台功能并入工作台待处理面板的「待审核事项」tab（超管可见智能体上架审批），侧边栏不再单独保留审核台入口。
- 工作台重构为可拖拽面板网格：顶部仅保留统计卡片（待办/待审查/进行中/待审核/我的案件）；功能面板统一外壳 `WorkbenchPanel.vue`，支持拖拽排序、自由添加/移除，布局 localStorage 持久化；面板含待处理（tab 融合待办任务/待审查成果/待审核事项）、通知、常用链接、月视图日程、任务统计、案件统计、智能体使用情况、运行中心（超管）。
- 数字警员页 `AgentManageView` 改为「数字警员 / 协助伙伴」双 tab，卡片样式不变，协助伙伴管理完整内嵌。
- 探索市场：新增「本周 / 本月」最受欢迎切换（默认本周，按时间窗过滤热度）；首页按「最受欢迎 → 数字警员 → 技能/工具/MCP/卡片 → 协助伙伴」分板块展示；技能/工具/MCP/卡片列表改为自适应多列紧凑网格、描述单行截断，卡片点击弹出详情弹窗（完整介绍/版本/依赖/启用/添加）；修复 skills 接口 `{success,data}` 包装未解包导致的 `skills.value.map` 崩溃。
...
- 新增办案复盘（模块 I.1，任务后反思 + 技能自修复待审流）：后端新增 `police_reflection_service.py` / `police_reflection_router.py` + `police_reflection_records` 表（ensure_business_schema 建表），提供 `POST /police/reflections/trigger`（模式A 规则化判定：对话过短跳过、「不记什么」清单过滤、含流程关键词生成技能沉淀草稿，一次可产出记忆审计+技能沉淀多条 draft）、`POST /police/reflections` 显式创建、`GET /police/reflections`（本人 + 超管全部，trigger_type/status 过滤）、`GET /{id}` 详情、`POST /{id}/review`（approve→applied 且技能沉淀自动调用模板服务入册 / reject→rejected；仅创建者或超管；已审阅不可重复操作），全部走审计链路；前端新增 `ReflectionReviewView.vue`（状态/类型筛选 + 列表 + 手动触发弹窗 + 确认入册/驳回操作），路由 `/police/reflections` + 侧边栏「办案复盘」菜单。合规红线：沉淀产物默认 draft/pending_review，民警确认后才入册，绝不自动改线上。已在 api-dev 容器冒烟验证 trigger→list→review（模板落库 id=20）→重复审阅拦截。
- 新增小南市场（模块 A，探索 + 发布 + 审核）：后端新增 `police_market_router.py` / `police_market_service.py`，提供 `GET /police/market/explore`（type=agent/partner/template 统一结构 + apply_mode：connect / equip_guided / install，keyword/category/分页）、`GET /police/market/{type}/{id}` 详情、`POST /police/market/{type}/{id}/apply`（复用 `police_agent_connections` 连接表，不复制警员）、`POST /police/market/publish`（agent/partner 复用 global→pending 共享审批流、模板直接上架，仅创建者/超管可发布）、`GET /police/market/pending` + `POST /police/market/{request_type}/{id}/approve`（仅超管），全部走审计链路；前端新增 `ExploreView.vue`（tabs/搜索/最受欢迎/卡片网格/分页/空状态）+ `MarketPublishDrawer.vue` 发布抽屉（数字民警完整路径，其余类型占位「即将支持」），路由 `/police/explore` + 侧边栏「探索市场」菜单，API 收敛到 `policeMarketApi`。MVP 仅「完整复刻」发布，「切片打包」为 P2。已在 api-dev 容器冒烟验证 explore/apply 真实跑通。
- 智能体头像统一本地化（公安内网零外网依赖）：新增 `web/public/avatars/officer-default.png` 统一占位图（用户提供彩色版，深蓝警帽/制服、透明背景），`policeAvatar.js` `getOfficerAvatar`/`resolveAgentAvatar` 统一回退到该图（已配置 avatar/icon 仍优先展示，兼容 emoji key 与原始 SVG 坏数据）；数字警员广场/档案页、协助伙伴管理/装备面板、智能体切换下拉、聊天子智能体头像、编辑弹窗预览、智能体档案/卡片头像均统一显示该占位；修复 `generateRandomPoliceAvatar` 返回原始 SVG 文本而非 data URI 导致头像无法渲染的 bug（`getPoliceAvatarById` 同步修复）。AgentCard `.card-avatar` 与 AgentProfileView `.ap-hero-avatar` 头像框改为透明底色、放大尺寸（88→104 / 156→184 / 移动 96→116）并保持与背景色块的间隙。补充 `policeAvatar.test.js` 7 项单测。
...
- 公安业务补齐（Phase 1 收尾）：审计统一收敛为 `write_audit_log`，补齐任务全部操作（创建/分配/开始/完成/审核）与证据上传/审核审计；证据管理新增 `download`/`preview` 端点，完成上传→下载闭环；任务流转规则新增增/查/删 API，可由配置驱动"完成任务自动建后续任务"；新增 `test/integration/api/test_police_business_router.py` 端到端测试，覆盖案件/任务/证据/流转引擎/审计/工作台主链路。
- 新增笔录导入与智能建案（Phase 2）：`police_transcript_service` 复用 `load_chat_model`(agnes-2.5-flash) 从笔录结构化提取案件概览与建议任务，复用 `ocr_service.parse_document` 解析 PDF/图片（best-effort，无 OCR 服务时仍支持纯文本）；新增 `POST /police/import/transcript`（分析）与 `/transcript/confirm`（确认建案+批量建任务），前端 `CaseImportView.vue` 提供上传/粘贴→编辑→确认闭环；LLM 输出用 `json_repair` 容错解析（修复未转义内嵌引号），确认建案时把自然语言案发时间归一化为日期入库（解析失败保留原文到 extra）。
- 任务管理整合进案件详情：独立「任务管理」看板页面从侧边栏移除，看板/表格双视图能力内嵌至 `CaseDetailView.vue` 的「任务」tab（自动按当前案件过滤，含 5 状态列卡片、类型标签、优先级、分配人）；路由 `/police/tasks` 保留供直接访问，`TaskBoardView.vue` 文件不动。
- 任务分配功能打通前后端全链路：后端 `get_case_detail` 补充案件成员 `username`（join users 表，修复前端下拉选项显示"未命名"）；前端创建任务弹窗新增「办案民警/智能体」类型切换（人类选项来自 caseData.members，智能体选项来自 `policeAgentApi.list`）；任务详情页 `TaskDetailView.vue` 新增「分配任务」按钮与弹窗（支持重新分配，调用 `POST /tasks/:id/assign`）。
- 新增数字警员广场与档案（Phase 2.5 数字警员平台，呼应 v1.3 需求）：前端 `officers/` 模块落地「数字警员广场」(画廊卡片：头像/工号/警衔/部门/状态/专长/能力标签/工作次数，支持搜索与类型/状态筛选) 与「数字警员档案」(能力矩阵/技能/SOP/工作记录时间线/成长轨迹/工作统计，含发起对话、编入专案组入口)；侧边栏新增「数字警员广场」★一级入口；复用现有 `GET /police/agents` 与 `GET /police/agents/{id}`(聚合 recent_runs/sops)，并提供「初始化预设警员」按钮调用 `POST /police/agents/seed`（系统已含笔录分析师/资金追踪师/调证生成师/法制审核官/案件编排官 5 名预设警员）。
- 修复任务分配功能不可用（任务列表与详情页均无分配入口）：① 案件详情「任务」tab 新增「分配」操作——表格行操作列与看板卡片底部均加「分配」按钮，点击打开与创建弹窗一致的「办案民警/智能体」切换选择弹窗，分配后自动刷新任务列表；② 任务详情页「分配任务」按钮原被 `status==='pending'` 卡死（非待处理任务看不到），放宽为「任务未完成（非 completed）前均可分配/改派」，覆盖待处理/进行中/待审核/已驳回；③ 经后端联调确认 `POST /tasks/:id/assign` 正常落库（`assignee_type`/`assignee_name` 持久化），分配链路后端无阻塞。**注意**：人类分配目标依赖案件成员（新建案件已自动把创建人加为 commander 成员），故新案件必有可分配民警；若案件无成员则人类下拉为空，可改派给智能体或先增补成员。

- 新增智能体市场（Phase 3）：复用 `police_agents` 表的 `is_template` 标记与新增 `category`/`install_count` 字段，预设 DA-001~004 作为市场模板（DA-005 案件编排官非模板）；后端新增 `GET /police/agents/templates`（按分类/关键词筛选、安装量降序）与 `POST /templates/{id}/install`（复制模板为新数字警员实例，自动生成工号并桥接 yuxi）；前端「智能体」页新增「智能体市场」Tab，9 分类标签 + 搜索 + 卡片网格 + 一键安装跳转到档案页。迁移补 `ALTER TABLE police_agents ADD COLUMN category/install_count`，市场路由须在 `/{agent_id}` 前注册避免被路径参数吞掉，安装错误改用 `JSONResponse` 返回正确状态码。
