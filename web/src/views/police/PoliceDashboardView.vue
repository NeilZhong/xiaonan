<script setup>
/**
 * ★ 公安工作台 — Plane 风格 Dashboard
 * POLICE_REQUIREMENTS §8.4.1
 */
...
const statCards = computed(() => [
  ...
  { title: '我的任务', value: policeStore.stats.my_tasks_total, icon: CheckCircleOutlined, color: '#38A169' },
])
...
onMounted(async () => {
  await Promise.all([
    ...
    policeStore.loadMyTasks(1, 5),
    policeStore.loadReviewTasks(1, 5),
  ])
})
</script>
    ...
    <div class="dashboard-grid">
      ...
      <div class="dashboard-panel">
        ...
        <div class="panel-body">
          <div v-if="policeStore.myTasks.length === 0" class="empty-state">
            <a-empty description="暂无待办任务" />
          </div>
          <div
            v-for="task in policeStore.myTasks"
            :key="task.id"
            class="task-item"
          ...
          >
            <div class="task-item-main">
              ...
              <div class="task-item-meta">
                <a-tag :color="statusColor[task.status]" size="small">{{ statusText[task.status] }}</a-tag>
                <a-tag :color="priorityColor[task.priority]" size="small">{{ task.priority }}</a-tag>
                <span class="task-item-type">{{ task.type }}</span>
              </div>
            </div>
            <div class="task-item-due" v-if="task.due_date">
              <a-tooltip title="截止时间">
                {{ task.due_date?.substring(5, 10) }}
              </a-tooltip>
            </div>
          </div>
        </div>
