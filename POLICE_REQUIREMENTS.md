...
| **系统管理员** | 平台级管理员，负责数字警察管理、运行时监控、全局配置；审计日志仅保留后端留痕（无独立审计台页面）。 |
...
> 说明：系统管理员在功能矩阵中通常拥有全部权限，但 **运行时控制台、数字警察管理** 是系统管理员独占功能，普通用户（包括案件指挥员）不可见。
...
> **v2.1 决策**：原规划的「全局审计台」独立查看页面已**取消**（审核功能已并入工作台，不再单独设审计台入口）。审计**留痕本身**作为合规与证据存证能力予以保留：记录仍写入 `police_audit_logs` 并支持程序化查询（见 §10.7），但不再提供管理端独立页面。

#### 6.8.1 可见性与访问

- **审计无独立页面**（v2.1 起取消独立审计台）；审计能力以「审计概览卡片 + 查看全部抽屉」形式并入**通用数据总览**（`DashboardView`），**仅系统管理员可见**（按 `userStore.isAdmin` 显隐）。
- **程序化访问**：系统管理员可通过审计查询 / 校验接口（§10.7）检索与核验；抽屉内的列表与哈希链校验即调用这些接口。
- **案件维度**：案件指挥员可在案件内查看本案件审计记录（§10.7 `/cases/{id}/audit/logs`），本期已实现。
...
- 运行时控制台、数字警察管理 **仅 system_admin 可见**。
...
| **Phase 2（进行中）** | 数字警察角色卡、案件中心、任务板、审核工作台、运行时控制台 | **P0 审批越权修复 + 审计查询/防篡改（后端，无独立前台）** |
...
| **P0** | 审计中间件自动捕获 ip/ua + 查询接口 | 后端（无独立前台页面） | 替换当前手写调用点 |
