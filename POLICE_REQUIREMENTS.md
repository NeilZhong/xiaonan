...
> **版本**: v1.3  
> **日期**: 2026-08-01  
> **状态**: 产品定位升级——从「案件项目管理」升级为「民警 + 数字警员协同办案」；明确数字警员=Yuxi Agent、技能=Yuxi Skill、SOP=编排层的概念模型；前端 UI 语言从参考 Plane 调整为参考 StaffDeck 数字员工广场 + 手绘风格  
...
**结论**：以语析 Yuxi 为技术底座、以 StaffDeck 为产品形态与 UI 语言、以 Octop 为安全能力参考，构成小南 v1.3 的「三位一体」选型框架。
...
**关键词**: 数字员工广场、手绘温度、卡片化、清晰专业

**v1.3 UI 参考调整**：一级产品隐喻参考 **StaffDeck** 的「数字员工」交互语言（数字员工广场 / 员工档案 / SOP / 工作记录），并采用更具亲和力的「手绘风」视觉（圆角、手绘图标、柔和插画感），让「数字警员」像真实同事一样被感知。任务/看板类协作视图仍参考 Plane 的布局交互。

参考要点：

- **数字员工广场**：卡片化画廊展示所有可用数字警员，显示在线/忙碌/离线状态、专长标签、本月工作统计（对应 4.3 数字警员中心）
- **员工档案**：头像 + 基本资料 + 能力矩阵 + 技能列表 + 工作记录日历 + 成长轨迹 + 对话日志（对应 4.3.4）
- **SOP 页**：本地 SOP 列表、版本、业务域、状态、调用次数、好评率/差评率（对应 4.8）
- **手绘风视觉**：圆角卡片、手绘风格图标与插画、柔和阴影，区别于冷硬的通用后台风格
- **专业配色**：以深蓝/藏青为主色调（公安行业属性），辅以灰白底色（色板见 8.2）
...
Phase 3: 数字警员协同与 SOP 驱动 (4周)
├── 资金追踪师（数字警员，fund_analysis Skill）
│   ├── 银行流水解析工具 (toolkits/bank_statement.py)
│   ├── 资金追踪 + 异常检测逻辑
│   └── 产出资金流向图 + 下级账户清单
├── 调证生成师（数字警员，evidence_collection Skill）
│   ├── 法律依据检索 (知识库 RAG)
│   └── 调取通知书自动生成
├── 法制审核官（数字警员，legal_review Skill）
│   ├── 复用语析审批中间件实现人机协作
│   └── 程序/证据/定性三维度审核
├── 案件编排官（数字警员，基于语析 SubAgents）
│   ├── 监听案件事件，按 SOP 调度各数字警员/技能
│   ├── 在专案中提示需开具的文书与待办工作项
│   └── 自动创建后续工作项
└── SOP 驱动的任务自动流转引擎 (police_sops + task_flow_rules + 事件监听)
...
| M3: 数字警员协同与 SOP 驱动 | 第 14 周 | 资金/调证/法制/编排数字警员 + SOP 任务自动流转 |
| M4: 知识图谱上线 | 第 17 周 | 知识库+知识图谱+可视化+图谱分析 |
| M5: 安全加固完成 | 第 20 周 | PII 脱敏+审计+隔离+渗透测试 |
| M6: 正式交付 | 第 23 周 | 完整平台 + 部署文档 + 培训材料 |
...
│   │   ├── stores/               # Pinia 状态管理
│   │   ├── views/                # 页面
│   │   │   ├── dashboard/        # 工作台 (民警待办/案件动态/数字警员动态)
