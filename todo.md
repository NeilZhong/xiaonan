...
> **最后更新**: 2026-08-04
> **当前里程碑**: M0 ✅ → M1 ✅ → M1.5 ✅ → M2 ✅ (闭环) → M3 ✅ 主体 (推进管线+任务模板) → **M3.5 📋 (案件记忆规划)** → M4 ⬜ (图谱未启) → M5 ⚠️ (安全部分) → M6 ⬜
> **开发文档**: [POLICE_REQUIREMENTS.md](./POLICE_REQUIREMENTS.md) **v1.6**（v1.5 文档—代码对齐版 + v1.6 案件记忆体系设计增量）
...
### v1.6 案件记忆 Case Memory 规划（2026-08-04）

> 方向：在小南中，**记忆主体是案件而非用户**——案件即「项目/任务/流程经验块」（对应 TencentDB-Agent-Memory L2 Scenario 理念，仅借鉴不整包引入）。采纳决策：①共享层(`case_id`)+个案层(`case_id`×`agent_id`)双层结构；②事件抽取+人工确认落盘；③结案归档只读；④允许跨案反哺智能体长期记忆。

- [x] **案件记忆 PRD 起草**：`docs/vibe/2026-08-04-case-memory-prd.md` v1.0 草稿（背景/EARS 需求/双层结构/触发点映射/数据模型/权限/验收/任务卡 T1–T10）
- [x] **需求文档回填**：`POLICE_REQUIREMENTS.md` 升 **v1.6**，新增 §4.12 案件记忆、§5.2.12 案件记忆块表、§7.2.11 案件记忆 API、Phase 3.5 路线图、M3.5 里程碑；同步记录 2026-08-04 代码变更（移除全局任务看板、任务模板配置权限收敛至超管）
- [ ] **案件记忆研发（T1–T10，待排期）**：详见 PRD §13 与文档 Phase 3.5
  - [ ] T1 数据模型 `police_case_memory_blocks` + `ensure_business_schema()` 建表
  - [ ] T2 案件记忆服务：共享层/个案层双层读写 + provenance 可逆白盒
  - [ ] T3 事件抽取管线：建案+笔录分析→共享层；任务完成→个案层（主触发）
  - [ ] T4 人工确认落盘（`pending_confirmation` 模式）
  - [ ] T5 召回：BM25+向量+RRF 检索替换「单文件全量注入」（解决 §0.2 技术债 5 的 64KB 截断）
  - [ ] T6 权限隔离：仅参案角色可见，跟随 `police_case_agents` 访问控制
...
| M3.5: 案件记忆上线 | 待排期 | 📋 规划中 | 双层案件记忆(共享层+个案层)+事件抽取确认+结案归档(P1)；检索召回+跨案反哺(P2) |
